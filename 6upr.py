...
class Bullet:
    ...
    def draw(self):
        pygame.draw.circle(self.screen, self.color, (round(self.x), round(self.y)), self.radius)
...
class Target:
    ...
    def draw(self):
        pygame.draw.circle(self.screen, self.color, (round(self.x), round(self.y)), self.radius)
        pygame.draw.circle(self.screen, BLACK, (round(self.x), round(self.y)), self.radius, 2)
...
class Cloud:
    ...
    def draw(self):
        pygame.draw.circle(self.screen, BLACK, (round(self.x + 40), round(self.y + 20)), 20)
    ...
    for bullet in tank.bullets:
        for cloud in [cloud1, cloud2]:
            for target in cloud.targets:
                if target.live and (
                        (target.x - bullet.x) ** 2 + (target.y - bullet.y) ** 2 <= (target.radius) ** 2):
                    target.live = 0
        ...
        for target in cloud1.targets + cloud2.targets:
            if tank.lives > 0 and (
                    (target.x - tank.body.x) ** 2 + (target.y - tank.body.y) ** 2 <= (target.radius + 10) ** 2):
                tank.lives -= 1
                finished = True  # Завершаем игру при столкновении
